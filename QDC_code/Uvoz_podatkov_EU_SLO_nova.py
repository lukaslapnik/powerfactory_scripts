...
app.PrintPlain(dfLoadP_EU) 
app.PrintPlain(dfLoadQ_EU)   
...
generators_adaptive = []
generators_pq = []
gen_list_slo = dfGenP_SLO.index.to_list()
gen_list_eu = dfGenP_EU.index.to_list()
...
for generator in app.GetCalcRelevantObjects("*.ElmSym"):
    ...
    if generator_name in gen_list_slo:
    ...
    elif generator_name in gen_list_eu:
...
for generator in app.GetCalcRelevantObjects("*.ElmSym"):
    ...
    for chaOld in generator.GetContents("usetp*.ChaVec"): chaOld.Delete()
...
# ALTERNATIVA, NAMESTO ADAPTIVE DELAMO TE GENERATORJE PQ

for i,generator in enumerate(generators_adaptive):
    generator_name = generator.GetAttribute("loc_name")
    #Klasična bremena/odjem
    app.PrintPlain(f"Proizvodnja {generator} kot PV brez station controllerja, nastavljeno na constV mode, št. {i+1}/{len(generators_adaptive)}")
    generator.av_mode = "constv"
    try:
        #Assign P vector
        # Remove old data
        if clear_old_data:
    ...
    try:
        ...
        if clear_old_data:
            for chaOld in generator.GetContents("usetp*.ChaVec"): chaOld.Delete() 
        # Assign controller to generator
        chaUsetp = generator.CreateObject("ChaVec", "usetp") 
        chaUsetp.SetAttribute("scale", timescale)
        chaUsetp.SetAttribute("vector", dfGenU_EU.loc[generator_name].to_list())
        chaUsetp.SetAttribute("usage", 2)
        if detailed_info: app.PrintPlain(f"Nastavil podatke U {chaUsetp} za {generator}")
    except:
        app.PrintWarn(f"Napaka nastavljanja U karakteristike za {generator}, preveri vhodne datoteke!")
